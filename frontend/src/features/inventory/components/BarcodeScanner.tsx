...
export default function MobilStokScanner({ onResult, onClose }: any) {
  ...
  useEffect(() => {
    // Tarama ayarlarını optimize et: Sadece yaygın barkod formatlarını tara (Hız kazandırır)
    const hints = new Map();
    const formats = [
    ...
    hints.set(DecodeHintType.TRY_HARDER, true); // Odaklanma sorunu için önemli
    ...
    try {
      ...
      const constraints: any = {
        video: {
          ...
          width: { ideal: 1280 },
          height: { ideal: 720 },
          // Bazı mobil tarayıcılarda odaklanmayı tetikler
      ...
      // Donanımsal Odaklanma Desteği (Advanced)
      ...
      readerRef.current.decodeFromStream(stream, videoRef.current, (res, err) => {
        if (res && !lock.current) {
          ...
          // 2 saniye bekleme süresi (üst üste okumayı engeller)
          setTimeout(() => {
            setProcessing(false);
            lock.current = false;
          }, 2000);
  ...
  return (
    <div style={{ width: '100%', maxWidth: '400px', margin: '0 auto', background: '#000', color: '#fff', fontFamily: 'sans-serif', minHeight: '100vh' }}>
      ...
      <div style={{ padding: '12px 16px', color: '#888', fontSize: '14px' }}>Sayım Ekranı</div>
      ...
      <div style={{ position: 'relative', width: '92%', height: '240px', margin: '0 auto', background: '#111', borderRadius: '12px', overflow: 'hidden' }}>
        ...
        {processing && (
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
            <span style={{ fontWeight: 'bold', letterSpacing: '1px' }}>OKUNDU...</span>
          </div>
        )}
